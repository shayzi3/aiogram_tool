...
class DependFilter(Filter):
    ...
    def get_depend_tool(self, data: dict[str, Any]) -> DependTool:
        ...
        if not isinstance(depend_tool, DependTool):
            raise NotFoundDependTool("Not found DependTool. Add it in setup")
        return depend_tool
        ...
        for depend in self._dependencies:
            ...
            try:
                params = await resolver.resolve_depend_params(
                    scope_object=scope_object, resolved_depends=set()
                )
                await resolver.depend_call(scope_object=scope_object, params=params)
            except DependExit:
                return False
        return True
