...
class SlidingWindowRateLimit(BaseRateLimit):
    ...
    async def execute(
        ...
        async with lock:
            ...
            user_time = [datetime.fromisoformat(date) for date in json.loads(times)]
            active_user_time = [
                timestamp
            ...
            if len(active_user_time) >= self.requests:
                if len(active_user_time) != len(user_time):
                    await storage.set_value(
                        key=key,
                        value=json.dumps(
                            [timestamp.isoformat() for timestamp in active_user_time]
                        ),
                    )

                await answer_callback(
                    event, self.time, self.time - (current_time - min(active_user_time))
                )
                return False
