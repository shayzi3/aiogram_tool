...
from aiogram_tool.tools.limit import RateLimitAnswer, RateLimitFilter, RateLimitTool
...
class MyAnswer(RateLimitAnswer):
    ...
    async def __call__(
        self, event: Message, window_time: timedelta, retry_after: timedelta
    ) -> None:
...
@pytest.fixture(
    scope="function", params=[MemoryLockStorage, FileLockStorage, AsyncRedisLockStorage]
)
async def local_storage_lock(request) -> AsyncGenerator[BaseLockStorage, None]:
    async with create_storage(
        cls=request.param, file_path="./local_test_lock.txt"
    ) as storage:
        yield storage
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        (
            FixedWindowRateLimit(requests=2, time=timedelta(seconds=1)),
            [["handle", "handle"]],
            False,
        ),
        (
            SlidingWindowRateLimit(requests=2, time=timedelta(seconds=1)),
...
async def test_with_setup(
    ...
    @my_dispatcher.message(RateLimitFilter(rate_limit=rate_limit))
    async def handle(message: Message) -> str:
        assert isinstance(message, Message)
        return "handle"

    assert await my_dispatcher.message_update(user_id) == "handle"
...
@pytest.mark.parametrize(
    argnames=("rate_limit",),
    argvalues=(
        (FixedWindowRateLimit(time=timedelta(seconds=1), requests=1),),
        (SlidingWindowRateLimit(time=timedelta(seconds=1), requests=1),),
        (TokenBucketRateLimit(bucket_size=1, current_tokens=1),),
    ),
)
async def test_all_users(
    ...
    @my_dispatcher.message(RateLimitFilter(rate_limit=rate_limit, all_users=True))
    async def handle(message: Message) -> str:
        assert isinstance(message, Message)
        return "handle"
...
async def test_limits_errors(
    rate_limit_class: BaseRateLimit, data: list[dict[str, Any]]
) -> None:
