...
async def test_depend_filter(
    ...
    async def depend_for_filter(
        context: Message, session: Session = Depends(get_session)
    ):
        session.storage["text"] = context.text

    @my_dispatcher.message(DependFilter(Depends(depend_for_filter)))
    async def handle(message: Message, session: Session = Depends(get_session)) -> str:
    ...
    handle_result = await my_dispatcher.message_update(
        text="Hello", dispatcher=my_dispatcher
    )
