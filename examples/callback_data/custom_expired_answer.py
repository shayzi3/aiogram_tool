...
from aiogram_tool.tools.callback_data import CallbackDataAnswer, LongCallbackData
...
async def get_keyboard() -> InlineKeyboardMarkup:
    callback_data = await TempData(
        operation_id="op_123", huge_log="L" * 120
    ).pack_long()
...
async def process_operation(query: CallbackQuery, callback_data: TempData):
    await query.answer(
        text=f"Operation succeeded! Len huge log: {len(callback_data.huge_log)}"
    )
